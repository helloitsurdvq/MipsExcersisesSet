# Prints the uppercase letter that appears most often in the string and the positions of that letter
.data
	cnt: .word 256
	s: .space 21
...
end_of_string:
	...
	lw $s2, ($t3) # $s2 = cnt[maxChar]
	li $s0, 66
	
check_max_loop:
	beq $s0, 91, after_check_max_loop
	la $t3, cnt
	mul $v0, $s0, 4
	add $t3, $t3, $v0 # $t3 = addr(cnt[i])
	lw $t4, ($t3) # $t4 = cnt[i]
	ble $t4, $s2, not_change
